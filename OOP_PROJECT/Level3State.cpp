...
void Level3State::setPlantFactoryTextures(AssetManager& Assets)
{
...
void Level3State::setZombieTextures(AssetManager& Assets)
{
...
void Level3State::handleAllPlantsCreation(sf::RenderWindow& window)
{
...
void Level3State::spawnZombies()
{
	...
	const int zombieTypes[numWaves][2] = { {0}, {1, 3} };
...
void Level3State::renderPlantFactory(sf::RenderWindow& window)
{
...
void Level3State::renderUI(sf::RenderWindow& window)
{
...
void Level3State::generateSuns(sf::RenderWindow& window, float deltaTime)
{
	for (int i = 0; i < 5; i++)
	{
...
void Level3State::handleSunCollection(sf::RenderWindow& window)
{
...
void Level3State::handlePlantRemoval(sf::RenderWindow& window)
{
...
void Level3State::handleBulletZombieCollision(sf::RenderWindow& window)
{
...
void Level3State::handlePlantZombieCollision()
{
	for (int i = 0; i < 4; ++i) // Iterate over shooter types
	{
		for (int j = 0; j < totalShooterInstancesCreated; ++j) // Iterate over shooter instances
...
void Level3State::handleLawnMowerCollision(float deltaTime)
{
...
void Level3State::Init(AssetManager& Assets)
{
	background.setTexture(Assets.getTexture(27));
	setPlantFactoryTextures(Assets);
	setZombieTextures(Assets);
	setUITextures(Assets);
}
void Level3State::HandleInput(StateMachine* machine, sf::RenderWindow& window)
{
...
void Level3State::Update(StateMachine* machine, float deltaTime)
{
	int shooterPlanttypes = 4;
	// For plant factory
	for (int k = 0; k < shooterPlanttypes; k++)
	...
	Level3State::spawnZombies();
	Level3State::handleLawnMowerCollision(deltaTime);
	...
	{
		machine->AddState(StateRef(new Level4State()), true);
	}
}
void Level3State::Draw(sf::RenderWindow& window, float deltaTime)
{
